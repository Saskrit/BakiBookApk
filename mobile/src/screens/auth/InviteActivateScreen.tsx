...
type Props = NativeStackScreenProps<RootStackParamList, 'InviteActivate'>;
...
export default function InviteActivateScreen({ navigation, route }: Props) {
  const { email, message: initialMessage } = route.params;
  const { activateInvite, resendInviteCode } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('auth.inviteActivateFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResend = async () => {
    ...
    setMessage('');
    try {
      const data = await resendInviteCode(email);
      setMessage(data.message || t('auth.inviteCodeResent'));
    } catch (err) {
      setError(err instanceof Error ? err.message : t('auth.inviteResendFailed'));
    } finally {
      setResending(false);
    }
  };
  ...
  return (
    <View style={authStyles.container}>
      ...
      <AuthKeyboardLayout>
        <Pressable onPress={() => navigation.replace('Login')} hitSlop={8} style={styles.backBtn}>
          <BackIcon />
        </Pressable>
        ...
        <View style={authStyles.card}>
          <Text style={authStyles.cardTitle}>{t('auth.inviteActivateTitle')}</Text>
          <Text style={authStyles.cardSubtitle}>{t('auth.inviteActivateSubtitle', { email })}</Text>
          {error ? <Text style={authStyles.error}>{error}</Text> : null}
          {message ? <Text style={styles.info}>{message}</Text> : null}

          <Text style={authStyles.label}>{t('auth.inviteCode')}</Text>
          ...
          >
            {loading ? (
              ...
              <>
                <Text style={authStyles.primaryBtnText}>{t('auth.inviteActivateSubmit')}</Text>
          ...
          <Pressable onPress={handleResend} disabled={resending} style={styles.resend}>
            <Text style={authStyles.altLink}>
              {resending ? t('common.sending') : t('auth.resendInviteCode')}
            </Text>
          </Pressable>
        </View>
      </AuthKeyboardLayout>
